...
class ToolManager:
    ...
    def parse_action_list(self, action_list: List[Dict[str, Any]]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        Args:
            action_list (List[Dict[str, Any]]): _description_

        Returns:
            List[Tuple[str, Dict[str, Any]]]: _description_
        """        
        ...
        for i, action in enumerate(action_list):
            if not isinstance(action, dict):
                print()
                raise ValueError(f"第{i+1}个action必须是字典格式, 实际内容是：\n{pformat(action)}")
        
            if "tool" not in action:
                raise ValueError(f"第{i+1}个action缺少'tool'字段, 实际内容是：\n{pformat(action)}")
 
            tool_name = action["tool"]
            
            if not isinstance(tool_name, str):
                raise ValueError(f"第{i+1}个action的'tool'字段必须是字符串, 实际内容是：\n{pformat(action)}")
